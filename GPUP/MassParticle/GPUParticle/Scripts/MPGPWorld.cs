...
{
    [SerializeField] public string integrate = "Integrate";
    ...
    public int m_max_particles = 512;
    ...
    void Awake()
    {
        ...
        collisionResult = new int[m_max_particles];
        m_renderer = GetComponent<MPGPRenderer>();
    ...
    void OnEnable()
    {
        ...
        hitBuf_ = new ComputeBuffer(m_max_particles, Marshal.SizeOf(typeof(Hit)), ComputeBufferType.Default);
        m_renderer.SetSize(m_particle_radius);
    }
    ...
    void Update()
    {
        ...
        m_buf_sphere_colliders.SetData(m_sphere_colliders.ToArray());
        m_sphere_colliders.Clear();
        m_buf_capsule_colliders.SetData(m_capsule_colliders.ToArray());
        m_capsule_colliders.Clear();
        m_buf_box_colliders.SetData(m_box_colliders.ToArray());
        m_box_colliders.Clear();
        m_buf_forces.SetData(m_forces.ToArray());
        m_forces.Clear();
        ...
        if (m_solver == MPGPWorld.Interaction.Impulse)
        {
            ...
            cs.SetBuffer(kernel, "pimd", m_buf_imd);
            cs.SetBuffer(kernel, "cells", m_buf_cells);
            cs.SetBuffer(kernel, "_Hits", hitBuf_);
            cs.SetVector("_PlayerPos", player.position);
            cs.SetVector("_BasePos", transform.position);
            cs.Dispatch(kernel, num_active_blocks, 1, 1);
        }
        ...
        //衝突判定配列を取得、走査
        hitBuf_.GetData(collisionResult);
        for (int i = 0; i < collisionResult.Length; i++)
        {
            if (collisionResult[i] > 0)
            {
                player.GetComponent<Player>().Hit(hitInfo);
                break; //同時ヒット防止
            }
        }
    }
